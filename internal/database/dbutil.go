...
func DBConnURI(dbName string) string {
	var (
		user     = config.GetEnv("TODO_DATABASE_USER", "postgres")
		password = config.GetEnv("TODO_DATABASE_PASSWORD", "")
		host     = config.GetEnv("TODO_DATABASE_HOST", "localhost")
		port     = config.GetEnv("TODO_DATABASE_PORT", "5432")
	)
...
// It returns an error if no connection could be made.
...
func ResetDB(ctx context.Context, db *DB) error {
	if err := db.Transact(ctx, sql.LevelDefault, func(tx *DB) error {
		if _, err := tx.Exec(ctx, `
			TRUNCATE tasks CASCADE;`); err != nil {
			return err
		}
		return nil
	}); err != nil {
